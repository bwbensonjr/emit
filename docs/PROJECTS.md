...
- **A library cannot export macros.** `define-syntax` works inside a library, but exporting a macro
  through the artifact is not supported; the derived forms reach your code through `(scheme base)`.
