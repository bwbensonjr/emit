...
(define (known-call-label p n)
  (let ([minimum-label (cadr (cdddr p))])
    (if (and (cadddr p) (= n (caddr p)) minimum-label) minimum-label (cadr p))))
...
(define (known-import sym n)
  (let ([p (assq sym *import-calls*)])
    (and p (if (cadddr p) (>= n (caddr p)) (= n (caddr p))) (known-call-label p n))))
...
(define (add-unit-assigned! name) (set! *unit-assigned* (cons name *unit-assigned*)))
...
(define (lower-program e . opt)
  ...
  (let* ([definition? (or (null? opt) (null? (cdr opt)) (cadr opt))]
         [entry
           (lower-top e definition?)]) ; top level: no locals, no free vars, no self
...
(define (lower e locals fmap self)
  ...
  (match e
    ...
    [(lambda ,params ,body) ; standalone -> make-closure
      (let* ([fvs (free-vars e)] [label (fresh-code-label)])
                     ...
                     [(lambda ,params ,lbody)
                       (let ([fvs (free-vars (caddr b))])
                         (hoist-code! label
                                      ...
                                      lbody
                                      fvs
                                      (car b)) ; self-name = binding
                         ;; captures lowered in the group scope (siblings visible)
                         (list (car b)
                               label
                               (map-lr (lambda (v) (lower v locals2 fmap self))
                                       fvs)))]))
                 cbinds
                 labels)])
         ...
         [lbody (lower body
                       ...
                       (and self-name
                            (list self-name
                                  ...
                                  (and (param-rest params)
                                       (minimum-entry-label label)))))])
