...
- [x] 1.1 Replace `repo_root()` (`src/emit.cpp:219`) with a support-file resolver taking a
...
- [x] 1.2 Point `discover_toolchain()` (`src/emit.cpp:980`) at the resolver for
      `tools/llvm-env.sh`, so the popen'd path is resolved rather than assembled from a root
- [x] 1.3 Point the `runtime.c` path (`src/emit.cpp:1142`) at the resolver for
      `src/runtime/runtime.c`; report a resolver miss by name rather than handing a nonexistent
      path to clang
- [x] 1.4 Extend `make install` to ship `tools/llvm-env.sh`, `tools/log.sh` (sourced by
...
- [x] 1.5 Update the install target's narration to count support files alongside the library
      sources, in `docs/OUTPUT.md` form
- [x] 1.6 Verify by hand: `make install PREFIX=<tmp>`, then from a project directory holding a
...
- [x] 2.1 Record the toolchain the build resolved as `-DEMIT_DEFAULT_CC` / `-DEMIT_DEFAULT_GC_INC`
...
- [x] 2.2 Confirm `DESTDIR` staging does not change the recorded values (they describe the build
      machine's toolchain, not the install tree) and that `PREFIX` does not either
- [x] 2.3 Add the third precedence layer to `discover_toolchain()`: explicit env, then
...
- [x] 2.4 When the compiled-in defaults are the layer used and the link fails, report the recorded
      path and identify it as a build-time default, rather than surfacing only clang's error
- [x] 2.5 Verify all three layers by hand: explicit `CC`/`GC_*` wins; with those unset a working
      `llvm-env.sh` wins over the recorded values; with discovery made to fail the recorded values
      are used

## 3. Manifest chaining (#44)

- [x] 3.1 Change `resolve_manifest()` (`src/emit.cpp:255`) to return an ordered **list**: an
...
- [x] 3.2 Resolve a library name by walking the list and taking the first manifest that names it;
...
- [x] 3.3 Keep program-entry lookup on the **first** manifest only, so
      `emit: no program entry in manifest <path>` keeps naming the project's own file (design D4)
- [x] 3.4 Make the REPL's eager preload span the chain (union of the manifests' libraries), keeping
      the existing `already`/`deferred` handling for a name a later manifest repeats (design D7)
- [x] 3.5 Update `say_manifest()` to narrate each resolved manifest in order, and name the manifest
...
- [x] 3.6 Measure `emit repl` startup from a project directory against an install, before and
...
- [x] 4.1 Extend `test/install-layout-tests.sh` (the #35 suite that already installs into a temp
...
- [x] 4.2 Add the chaining cases to the same suite: a project manifest that names only its own
...
- [x] 4.4 Confirm the suite stays Chez-free and keeps its place in `run-all-tests.sh:71`; widen its
      ...
      the manifest lookup
- [x] 4.5 Run the full `./run-all-tests.sh`, and `./run-dev-tests.sh` for the Chez-gated byte-identity
      suites — the guard that the `resolve_manifest()` refactor is transparent in a checkout

## 5. Docs and downstream

- [x] 5.1 `docs/MODULES.md`: describe the chain in the manifest section — searched candidates
      extend, explicit requests do not, program lookup does not chain
- [x] 5.2 `docs/PROJECTS.md`: remove the #44 sharp-edge note and replace it with what a project
      manifest now needs (only its own entries)
- [x] 5.3 `openspec/changes/homebrew-tap-distribution/tasks.md`: redirect task 3.2 from designing
...
- [x] 5.4 `docs/PERFORMANCE.md`: record the deferred prebuilt-runtime idea (shipping `runtime.o` or
...
- [x] 5.5 `openspec/explorations/library-sources-and-artifacts.md`: mark open question 3 as
