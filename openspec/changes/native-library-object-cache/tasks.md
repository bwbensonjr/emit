...
- [ ] 1.1 Record empty-portable-cache, warm-IR-cache, and first-import baselines for the baked set,
  ...
  only an aggregate.
- [ ] 1.2 Build a focused ORC probe using the supported LLVM headers that captures the object buffer
...
- [ ] 2.1 Add a canonical JIT signature covering the native format, Emit executable, LLVM identity,
...
- [ ] 2.2 Add distinct disk-unit and baked-set native entry layouts derived from validated portable
...
- [ ] 2.3 Implement atomic object/metadata writes, a last-written completeness stamp, object-content
  ...
  entering a JIT.
- [ ] 2.4 Extend the cache test fixtures with safe native-entry inspection and mutation helpers;
...
- [ ] 3.1 Isolate LLVM native-object capture and admission behind one host adapter, wire cold IR
...
- [ ] 3.2 Add host-level native lookup before library IR parsing and admit a valid hit through ORC's
...
- [ ] 3.3 Make every native miss or pre-admission failure fall through to the existing portable IR
...
- [ ] 3.4 Bypass native lookup and capture when `--dump-all` requests library compilation stages;
...
- [ ] 4.1 Enable native objects for full library units in `emit run`; verify cold execution creates
...
- [ ] 4.2 Enable native objects for baked and manifest libraries in `emit repl` without changing
...
- [ ] 4.3 Exercise conventional-path on-demand registration with a mixed native-hit/IR-miss dependency
  closure; verify transaction failure publishes no imports and the next valid REPL form still runs.
- [ ] 4.4 Keep `emit build` and `emit lib` outside native consumption; verify delivered executable
...
- [ ] 4.5 Verify dev-to-ship fidelity across cached and uncached `-O0`, `-O1`, and `-O2` sessions for
...
- [ ] 5.1 Narrate native misses, generation, and reuse with the library, profile, bytes, and relevant
  ...
  verbosity.
- [ ] 5.2 Document the portable and native cache tiers, key inputs, local-install behavior, safe
  ...
  built command.
- [ ] 5.3 Re-run the baseline protocol with native reuse enabled and record cold generation, warm
...
- [ ] 6.1 Register the focused native-object cache suite in both narrated test workflows and verify
  suite discovery counts, stderr narration, `EMIT_VERBOSITY` behavior, and temporary-cache isolation.
- [ ] 6.2 Build and run the focused artifact-cache, unit-pipeline, JIT-optimization, REPL module,
...
- [ ] 6.3 Verify the LLVM compatibility adapter builds and the focused native suite passes against
...
- [ ] 6.4 Run `make format-check`, `./run-all-tests.sh`, and, after the implementation commit makes
