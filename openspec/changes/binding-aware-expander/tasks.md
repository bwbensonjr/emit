...
- [ ] 7.3 Run `./run-all-tests.sh` (~27 min) and `./run-dev-tests.sh` (~25 min; run its suites individually if the command timeout is shorter than a suite). `test/prelude-base-run-tests.sh`'s driver-vs-run-door byte identity and `test/self-host-fixpoint.sh` are the two that would catch a path asymmetry introduced by task 4.
- [ ] 7.4 Commit, then run `test/trust-check.sh` — it `[SKIP]`s while `bootstrap/` is dirty by design.
...
- [ ] 7.6 Close #103, #92, and #79 from the commit (`Fixes #103`, `Fixes #92`, `Fixes #79`), noting in #103 that the top-level case is handled by user-wins pruning and in #92 that the literal test is use-site-only. File a follow-up issue for anything the non-goals leave behind that the work made newly legible.
