...
- **A string port's buffer becomes collectable.** The memstream buffer is `malloc`'d outside
  the GC heap and is never freed today — a documented leak bounded by the number of string
  ports a program opens. A GC-heap buffer is reclaimed when the port becomes unreachable.
...
- `io-ports`: the "Textual output ports over files and strings" requirement gains two
  guarantees it does not currently make — that the cost of opening a string output port is
  independent of how many output ports are already live, and that a string port's storage is
  reclaimable rather than held for the life of the process.
