...
- [ ] 3.1 Generalize `cmp-chain-eta` (`src/parse.ss`) to build its pairwise test from the same
...
- [ ] 3.2 Add the **eta-only** entry marker to `*integrable*` and teach `eta-integrable` /
...
- [ ] 3.3 Add the eta-only entries for `>`, `<=`, `>=` over `%<` and `%=`, and confirm
      `(map car *integrable*)` flowing into `compute-known` and `*repl-known*` binds the three
      names on both doors.
- [ ] 3.4 Verify operator position is byte-for-byte unchanged: compile a program using
      `> <= >=` in operator position before and after, and diff the emitted IR to prove no drift.
- [ ] 3.5 Make `max` variadic in `src/prelude.scm` and add `min`, both with R7RS contagion —
...
- [ ] 3.6 Tests: `(map > …)`, `(apply >= …)`, `(apply <= …)`, `(max 1 2 3)`, `(min 1 2)`,
      `(max 3 4.0)`, `(max 3.0 4)`, `(min 3 4.0)`, `(apply max …)`, `(map min … …)`, and the
      operator-position results for all five comparisons.
- [ ] 3.7 Check macro hygiene: a `syntax-rules` template mentioning `>` or `<=` must treat it as a
      known binding (the reason `*repl-known*` unions the integrable names) and must not rename it
      to an unbound `>.0`.
- [ ] 3.8 `make regen` (plain) and confirm convergence; regenerate `lib/scheme/base.sld` and run
      `test/scheme-base-gen-check.sh`.

## 4. The (scheme base) numeric inventory (#27, design D4/D6/D7)

- [x] 4.1 Pre-flight check: grep the compiler's own source for each of the ~30 new prelude names to
...
- [ ] 4.2 Predicates in `src/prelude.scm`: `complex?`, `rational?` (finite real), `exact-integer?`,
...
- [ ] 4.3 `abs`, `square`, and variadic `gcd`/`lcm` (identities `0` and `1`, non-negative results),
      exact for exact arguments, with `(abs FIXNUM_MIN)` and large `lcm` reaching the overflow
      diagnostic rather than wrapping.
- [ ] 4.4 `expt`: exact by repeated squaring for an exact base and non-negative exact integer
      exponent; inexact for a negative exponent; `(expt 0 0)` = `1`; overflow traps.
- [ ] 4.5 `exact-integer-sqrt` returning two values — resolve the design's open question by
      confirming the landed `multiple-values` machinery supports this from prelude Scheme before
      committing to the shape.
- [ ] 4.6 The rounding family: `floor`, `ceiling`, `truncate`, `round` — identity on exact
...
- [ ] 4.7 The R7RS division operators over the existing primitives: `truncate-quotient`,
...
- [ ] 4.8 `numerator` / `denominator` restricted to integer-valued arguments, trapping otherwise.
- [ ] 4.9 The R7RS conversion spellings `exact` and `inexact` as equivalents of
      `inexact->exact` / `exact->inexact`, retaining the R5RS names.
- [ ] 4.10 `number->string` radix argument (2/8/10/16 for exact integers; non-decimal radix with an
      inexact argument is an error) and `string->number` reusing the reader's classifier and
      parsers, returning `#f` on failure.
- [ ] 4.11 Tests for every scenario in the `core-language` delta, including the exactness
      assertions (`(exact? (round 5))`, `(exact? (gcd 32 -36))`), the overflow traps, and the
      `string->number` / `number->string` round trips.
- [ ] 4.12 Regenerate `lib/scheme/base.sld`, run `test/scheme-base-gen-check.sh`, and measure a
...
- [ ] 4.13 `make regen` and confirm the fixed point converges with the enlarged prelude.

## 5. The non-finite reader tokens (#25, design D8)

- [ ] 5.1 Recognize `+inf.0`, `-inf.0`, `+nan.0` in `rd-atom` (`src/prelude.scm`) before the flonum
      classifier runs, as three exact literal-string cases.
- [ ] 5.2 Make the same three tokens numbers in the bootstrap reader's `const` path
      (`src/parse.ss`), so the two readers agree on what a datum means.
- [ ] 5.3 Tests: `(number? (read-from-string "+inf.0"))` and its two siblings; the
...
- [ ] 5.4 Confirm the deferred half of #25 is untouched and still reported clearly: `#x1f`, `#e1.0`,
      and `1/2` behave as they do today (no silent change), and the issue records what remains.

## 6. The (scheme inexact) library (design D5)

- [ ] 6.1 Stage 1 of the bootstrap: add the libm runtime functions (`rt_sqrt`, `rt_exp`, `rt_log`,
      the trig family, `rt_finite_p`, `rt_nan_p`) to `src/runtime/runtime.c`, their `prim-table`
      entries and `declare` lines to `src/emit.ss`, and the `%`-names to `*prims*` in
      `src/parse.ss` — no call sites yet — then `make regen` so the committed seed learns the heads.
- [ ] 6.2 Re-record `test/module-scaffold-baseline.sha256` if the new `declare` lines drift every
      module's IR hash, with a before/after capture proving the drift is only those lines.
- [ ] 6.3 Run the full dev suite and commit `bootstrap/*.ll` at this stable point, then re-run
      `test/trust-check.sh` (it skips while `bootstrap/` is dirty).
- [ ] 6.4 Write `lib/scheme/inexact.sld` by hand — `finite?`, `infinite?`, `nan?`, `exp`, `log`
...
- [ ] 6.5 Add the `(scheme inexact)` entry to `emit-libs.scm` — the first non-`(scheme base)`
      library in the default manifest.
- [ ] 6.6 Verify the library on all three doors: `emit run`, `emit repl`, and an `emit build`
      executable, each importing `(scheme inexact)` and agreeing on results.
- [ ] 6.7 Verify the negative cases: without the import, `sqrt` is an unbound variable; and a
      program defining its own `sqrt` without the import uses its own.
- [ ] 6.8 Tests for the IEEE domain behaviour — `(sqrt -1.0)`, `(log -1.0)`, `(asin 2.0)` are NaN
...
- [ ] 6.9 Measure `emit run` startup on a trivial program before and after the manifest entry, to
      confirm the eager preload cost stays inside the noise as D5 measured.
- [ ] 6.10 Stage 2 regen and a full dev-suite run; commit `bootstrap/*.ll` and re-run the
      trust-check.

## 7. Specs, docs, and issue hygiene

- [ ] 7.1 Sync the delta specs into `openspec/specs/` (`core-language`, `aot-codegen`, and the new
      `inexact-math`) and validate.
- [ ] 7.2 Document the new primitives in `docs/PRIMITIVES.md` — the libm `%`-ops in the
...
- [ ] 7.3 Update `docs/MODULES.md`: `(scheme inexact)` as a second standard library, the default
...
- [ ] 7.4 Record in `docs/PERFORMANCE.md` that `<=`/`>=` still cost two comparisons in operator
      position (design D2's rejected alternative C), so the real-primitive option is captured with
      its rationale rather than lost.
- [ ] 7.5 Refresh `docs/COMPLEXITY.md` via the complexity-catalogue skill.
- [ ] 7.6 Close #23, #24, #26 and the non-finite-token half of #25; update #25 to cover only the
...
- [ ] 7.7 Reference the issues from the fixing commits (`Fixes #N`), one commit per task group.
