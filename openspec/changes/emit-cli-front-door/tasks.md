...
- [x] 1.1 Accept `--help` / `-h` in `main`'s verb dispatch (`src/emit.cpp:1251-1268`) before the
      unknown-verb branch: print the usage summary and exit 0
- [x] 1.2 Give `usage()` (`src/emit.cpp:1239`) a destination argument so the same text goes to
      stdout when requested and stderr when it accompanies an error (design D1)
- [x] 1.3 Leave the error paths as they are: no verb exits 1, unknown verb exits 2, both narrating
      on stderr

## 2. Per-verb help and option validation (#43)

- [x] 2.1 Write a usage line per verb — argument form, its own options, the shared `--dump` /
      `--dump-all` — with `emit lib`'s existing line (`src/emit.cpp:1168`) as the first of four
      rather than a special case (design D2)
- [x] 2.2 Accept `--help` / `-h` in the option loops of `emit_run` (`:569`), `emit_build` (`:1079`),
...
- [x] 3.1 Add the `is_unspec` guard to the runtime's value print
      (`src/runtime/runtime.c:1843-1845`, the non-`RT_FILTER_MAIN` branch of `main`), suppressing
      the written form **and** the newline
- [x] 3.2 Add the same guard to the host's in-process run path in `src/emit.cpp`, so `emit run` and
      a delivered executable stay byte-identical on stdout (design D5)
- [x] 3.3 Confirm the guard is in the report, not the printer: `(write (if #f #f))` and
      `(display (if #f #f))` still render `#<unspecified>` — the same separation the REPL keeps
      between `run_thunk` and `print_val`
- [x] 3.4 Confirm `#f` and `()` still print as final values
- [x] 3.5 Leave `RT_FILTER_MAIN` untouched — it remains the separate compile-time mode that
      suppresses *every* final value, and `schemec` still builds with it (`Makefile:99`)

## 4. Move the pinned expectations

- [x] 4.1 `demos/mandelbrot.expected`: drop the trailing `#<unspecified>` line
- [x] 4.2 `test/io-primitives-tests.sh`: update the three expectations at `:47`, `:50`, `:53`
...
- [x] 4.3 Confirm `demos/run-tests.sh:184` does **not** move — its two `#<unspecified>` tokens come
      from explicit writes and the program's final value is a list (verified before this change)
- [x] 4.4 Confirm `test/repl-interactive-tests.sh:170` does **not** move — it pins the REPL rule
      this change generalizes

## 5. Tests

- [x] 5.1 Add help cases to a CLI suite: top-level `--help` and `-h` exit 0 on stdout; each verb's
...
- [x] 5.5 Run `./run-all-tests.sh` and `./run-dev-tests.sh`; confirm
...
- [x] 6.1 `docs/PROJECTS.md`: remove the explanation of the trailing `#<unspecified>`, which the
