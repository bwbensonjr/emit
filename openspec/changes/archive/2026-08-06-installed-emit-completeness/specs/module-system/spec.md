...
**Locating the manifest.** every path SHALL locate manifests by the same ordered procedure:
...
given but names a file that does not exist, the path SHALL report that named file as missing rather
...
- **THEN** the path reports no program entry, naming the project's own manifest, and does not search
