...
The module system has three paths, all sharing one compile-unit core:

```
                       manifest + files          compile-unit core
                       ────────────────          ─────────────────
   AOT path   bin/scheme-compile / compile.ss  → build-modular-artifacts*
     (Chez or --emit)   reads manifest, topo-loads units, then
                        compile-program-with-imports(prog, direct-tables, order)

   REPL  src/repl/host.cpp                → repl-core.ss modes
   ...
   `emit run` command   src/run.cpp                       → entry-embed.scm  ◀── THE GAP
...
the AOT path compiles the *program* against its direct imports' export tables via
...
- **Reuse, not fork**: drive the same manifest resolution + compile-unit core the AOT and
  REPLs use; add no second resolution path.
- Byte-identical program/unit modules vs the AOT path (fidelity by construction).
...
program text and calls the **same** `compile-program-with-imports` the AOT path calls
...
import closure in topological order. Same function + same inputs ⇒ the run-path program module
...
- **The host must JIT-add each unit module WITHOUT running its `__init`.** The program module's
  ...
  `emit run` command. Running both would double-initialize.
- **`init-libs` (the transitive closure in topo order) is computed in-language.** The driver's
  ...
  `with-scheme-base`), matching the AOT path.
...
from `src/prelude.scm`) and is listed in every manifest, and the AOT/REPLs build it from
...
imports come from the manifest via the host preload. Byte-identity with the AOT path still
...
importing a user library requires a manifest (as the AOT/REPLs do); a plain program never
...
linked by `make all` from `run.o + embed-repl.ll` (the dispatched, module-aware path) and never
...
- **[The new program-with-imports mode is the one genuinely new compiler surface]** → keep it
  ...
  `compile-program-with-imports`; add a byte-identity test (run-path program module ==
  AOT-path `prog.ll`) so any divergence fails loudly.
...
- **[Missing-from-manifest / cycle errors]** → reuse the REPL loader's fixpoint-with-no-
  progress detection so the `emit run` command reports the same diagnostic rather than looping.
