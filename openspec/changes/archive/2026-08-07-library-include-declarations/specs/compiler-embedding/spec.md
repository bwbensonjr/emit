...
### Requirement: A path installs the compiler's source reader
...
reader the **path** installed, taking the filename as written in the source and returning its
top-level forms. Resolution of a filename to a location, and the reading itself, SHALL belong to the
path, so the same core serves the Chez-hosted driver and the Chez-free binary without either one's
...
A path that installs no reader SHALL still fail comprehensibly: an inclusion attempted without an
...
#### Scenario: every path reads an included file

- **WHEN** the same library using `include` is compiled by the Chez driver and by each Chez-free
  path
- **THEN** each path reads the included file through its own installed reader and produces the same
  unit
...
### Requirement: A path tells the compiler where the source it submits came from

A path SHALL be able to tell the embedded compiler the location of the source it is about to submit,
through the same mode-based entry protocol used for the path's other operations, before submitting
...
appearing in the source. When a path submits source with no location — text read from standard input
...
so that a path's dependency resolution sees an import that arrived through
`include-library-declarations` or `cond-expand`.

#### Scenario: A library resolves its includes from outside the project directory

- **WHEN** a path compiles a manifest library that includes a file beside it, from a working
...
- **WHEN** a program or library is piped to a path with no filename
...
- **THEN** the path's dependency walk reaches the dependency and preloads it, rather than reporting
