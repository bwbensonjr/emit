...
When a path's diagnostic reports a value the user wrote — a library name, a form, a literal — it
...
This applies to error irritants on every path, not only the interactive one: the paths share one
...
- **WHEN** any path reports an error whose irritant is a list
- **THEN** the diagnostic renders that list's external representation

#### Scenario: A vector irritant renders

- **WHEN** a path reports an error whose irritant is a vector or bytevector
