...
- [x] 7.3 Run `./run-all-tests.sh` (~27 min) and `./run-dev-tests.sh` (~25 min; run its suites individually if the command timeout is shorter than a suite). `test/prelude-base-run-tests.sh`'s driver-vs-run-path byte identity and `test/self-host-fixpoint.sh` are the two that would catch a path asymmetry introduced by task 4.
