...
registered on the run and `emit build` commands but **not** on the REPL or `emit lib` paths. Those two paths
...
project's stated primary development loop (`CLAUDE.md`), and `emit lib` is the compile-unit path;
...
have to know. The run and `emit build` commands already need no such entry, which is exactly the asymmetry to
...
is the thing a first release is *for*, and both defects are in the paths a new user meets first.

## What Changes

- **Register the baked set on the REPL.** `emit repl` SHALL build and register the baked set
  ...
  `(scheme base)` loads.
- **Resolve a lone library's imports, on every path.** `compile-library-form` (`src/core.ss`)
...
- **A manifest entry naming a baked member becomes a no-op on every path.** The
  already-loaded guard in `repl-load-library-text` that makes the `emit run` command tolerate a manifest
...
- **Add user documentation: `docs/PROJECTS.md`** — how to develop and compile your own Scheme
  ...
  limits. Written against the fixed paths, so it needs no workaround paragraphs, and verified by
  building the project it describes.
- **Drop the leaked `repl:` prefix** from diagnostics on the non-REPLs, so `emit lib` reports
...
- **Name an unreadable manifest source instead of crashing.** A manifest entry whose `(source …)`
  ...
  report the unreadable path and name it — the REPL keeping the session, the `emit run` command exiting
...
### New Capabilities
<!-- None. This makes an existing guarantee hold on two more paths; the documentation is a
     deliverable, not a spec'd capability. -->

### Modified Capabilities
- `module-system`: the baked-set requirement gains that **every path** registers the set before
  consulting the manifest, and that a manifest entry naming a baked member resolves to the baked
  one rather than loading a second copy. The REPL-path requirement changes: the session's standard
...
- `emit-cli`: `emit lib` SHALL compile a library whose body resolves names through its imports —
  ...
  from the non-REPLs SHALL not carry the `repl:` prefix.
...
- **`src/repl-core.ss`** — mode 8's payload carries one initializer symbol **per member** in the
  modules' dependency order (the `emit run` command ignores the field today, so this is additive); the
