...
An import the path cannot resolve — a library that is neither a member of the baked set nor named in
...
### Requirement: A path's diagnostics name that path, not the compiler's internal mode

A diagnostic a path prints SHALL be prefixed by the invoked path's name and SHALL NOT carry the
