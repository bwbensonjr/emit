...
this capability. Specifically, `Makefile` targets, the shipped `emit` binary's four commands
...
every path of the shipped compiler SHALL honor the project's single verbosity control at
...
#### Scenario: Quiet silences a path completely

- **WHEN** a path is run at the `quiet` level
...
- **WHEN** a path that writes a data payload to standard output is run at each verbosity
  level in turn
