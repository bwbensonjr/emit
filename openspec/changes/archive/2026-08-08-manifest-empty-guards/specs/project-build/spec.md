...
1. **No manifest was found.** The message SHALL say so, and SHALL name the manifest filename the
   path searched for, rather than naming an empty path.
...
Because a program entry is what `emit build` builds, cases 1 and 2 SHALL be errors for this path
even though "Finding no manifest at all SHALL remain non-fatal" holds for paths that only resolve
imports.
...
- **WHEN** the user runs `emit build hello.scm` in a directory whose manifest is absent or
  entryless
- **THEN** the path reports the applicable cause above and exits non-zero, rather than terminating
  on a signal
