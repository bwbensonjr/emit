...
- [x] 1.4 Run `./run-all-tests.sh` and `./run-dev-tests.sh` for the primitive-only phase; verify existing hash tables and all compiler paths remain green before beginning the second source-edit phase
...
- [x] 2.4 Add development, REPL, AOT, and backend-equivalence coverage for both table modes; verify the same programs produce identical values and effects through every path
