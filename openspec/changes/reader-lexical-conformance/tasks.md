> **Regen barrier** (`CLAUDE.md`): `src/prelude.scm`, `src/prelude-surface.scm`, and
> `src/repl-core.ss` are all baked into the binaries, so every source edit below must be finished
> before `make regen`. Iterate with `chez --libdirs src --script src/compile.ss` meanwhile.
>
> **This change moves emitted IR for every program** (the reader is part of `(scheme base)`), unlike
> the include change that preceded it. Do not land it alongside another IR-shaping change: the
> `test/module-scaffold-baseline.sha256` re-record has to be attributable to exactly one cause.

## 1. Block comments (issue #59, design D1/D2)

- [ ] 1.1 Teach `rd-skip-ws` (`src/prelude.scm:855`) to skip `#| … |#` with a **nesting depth
      counter**, so `#| a #| b |# c |#` is one comment.
- [ ] 1.2 Return the sentinel `-1` for an unterminated block comment rather than skipping to the end
      of input (design D2). The `rd-*` layer performs no raising — design D10 of
      `scheme-base-partition` keeps the handler chain out of the substrate — so the condition
      travels as a value.
- [ ] 1.3 Propagate the sentinel through `rd-datum` / `rd-list` without arithmetic on it, and raise
      at the three entry points that can: `read-from-string` (`:1080`), `read-all-from-string`
      (`:1090`), and `read` over a port (`:1238`). The message names the position the comment opened
      at.
- [ ] 1.4 Confirm a comment inside a **string literal** is still string content — `rd-string` runs
      before any of this, but the case is cheap to get wrong and cheap to pin.

## 2. Datum comments (issue #59, design D1)

- [ ] 2.1 Add `#;` to `rd-datum`: read the next datum, discard it, read the one after. Stacking
      (`#;#;a b c`) falls out of the recursion.
- [ ] 2.2 Add it to `rd-list`'s element loop as well, so `(1 #;2 3)` and `(1 2 #;3)` both work —
      including immediately before the closing paren, where there is no following element.
- [ ] 2.3 `#;` with nothing after it is the same unterminated case as 1.2: sentinel out, error at
      the entry point, *incomplete* at the prompt.

## 3. Number prefixes and rational literals (issue #25, design D3/D4)

- [ ] 3.1 Add a prefix scanner to the token path: at most one radix prefix (`#b` `#o` `#d` `#x`) and
      at most one exactness prefix (`#e` `#i`), in either order, either case.
- [ ] 3.2 Route the body through the existing classifiers — `rd-numeric?` / `rd-parse-int` for
      decimal integers, `%string->int` / `%radix-digits` for other radices, `rd-flonum?` /
      `%string->flonum` for decimals — so no second numeric grammar appears.
- [ ] 3.3 Enforce that a decimal point or exponent is radix-10 only; `#x1.8` is reported as invalid
      number syntax naming the token.
- [ ] 3.4 `#i` yields the inexact value; `#e` yields the exact one where it exists.
- [ ] 3.5 Report rational literal syntax `n/m` outright, naming the literal (design D4) — every
      `n/m`, including `4/2` and `#i1/2` whose values are representable, and `#e0.5`. Never a symbol.
      The message names the alternatives (`0.5`, `(/ 1 2)`).
- [ ] 3.6 A `#`-token that is none of the recognized forms is reported as unrecognized syntax naming
      it, instead of falling through `rd-hash`'s `[else]` arm into `string->symbol` — the arm that
      produced `unbound variable x1f` and `unbound variable |`.
- [ ] 3.7 Extend `string->number` (`:506`) through the same scanner, with a prefix in the text
      winning over the `radix` argument, and returning `#f` — not raising — where the reader reports
      a restriction (R7RS §6.2.6).

## 4. Bar-quoted identifiers, both halves (design D7)

- [ ] 4.1 Reader: `|` opens an identifier that runs to the matching `|`, with `\|` and `\xHH;`
      escapes. The result is an ordinary interned symbol — `(eq? '|foo| 'foo)` is `#t` — so this is a
      new lexeme, not a new type. Unterminated uses the same sentinel as 1.2.
- [ ] 4.2 Printer (`print_val`, `src/runtime/runtime.c`): write style emits `|…|` around a symbol
      whose name would not read back — empty, or containing whitespace, a delimiter, `|`, or `"`, or
      starting like a number or another datum — escaping `|` and `\` inside. `display` unchanged.
- [ ] 4.3 Confirm no symbol any existing test prints changes shape: the predicate must fire only for
      names that genuinely need bars, or every dump and REPL echo in the suites moves.
- [ ] 4.4 Round trip: `(write (string->symbol "a b"))` then reading that text yields an `eq?` symbol.
- [ ] 4.5 Leave `render-datum` (`src/core.ss`) alone but note the coupling in its comment: it writes
      `.exports` artifacts that BOTH hosts read back, and every name it writes today is a mangled
      identifier that needs no bars (design D7's closing note).

## 5. The interactive probe (issue #59 item 3, design D5)

- [ ] 5.1 Mirror both comment forms in `fc-hash` / `fc-datum` / `fc-list` (`src/repl-core.ss:679+`).
      The shared `rd-skip-ws` covers only a comment in leading position; the probe walks the rest of
      the form itself.
- [ ] 5.2 Map the unterminated sentinel to `fc-incomplete`, not `fc-malformed`, so a block comment
      can be typed across lines at the prompt.
- [ ] 5.3 Check the probe and the reader agree on the same text: anything the reader consumes as one
      datum, the probe reports complete with the same extent.

## 6. Surface bookkeeping and generated libraries (design D6)

- [ ] 6.1 Add every new `rd-*` helper to **both** `*scheme-base-private*` and `*substrate-rehomed*`
      in `src/prelude-surface.scm`. A definition in neither fails
      `test/scheme-base-surface-check.sh`; the file's reader comment already says "issue #25 will
      change these".
- [ ] 6.2 Regenerate `lib/scheme/base.sld`, `lib/scheme/read.sld`, and `lib/emit/internal.sld` with
      `tools/gen-scheme-base.ss` and commit them — they are committed build inputs
      (`tools/regen.sh` derives `bootstrap/scheme.base.ll` from `base.sld`).

## 7. Tests

- [ ] 7.1 Reader cases in `test/read-all-tests.ss`: nested block comments, a comment in each
      position (top level, inside a list, between quote and datum), `#;` including before a closing
      paren and stacked, comment syntax inside a string, and the unterminated error.
- [ ] 7.2 Numeric cases in `test/numeric-conformance-tests.sh`: each radix prefix, case
      insensitivity, a sign after a prefix, both prefix orders, `#x1.8` rejected, `1/2` reported,
      `4/2` integral, `#i1/2`, and the `#x` + `number->string` round trip.
- [ ] 7.3 `string->number` cases beside them, including a prefix beating the radix argument and
      `#f` where the reader raises.
- [ ] 7.4 Probe cases in `test/repl-interactive-tests.sh`: a multi-line block comment typed across
      lines is *incomplete* then completes; `(list 1 #;` is incomplete; a session survives an
      unterminated comment and evaluates the next form.
- [ ] 7.5 Restore the block comment in `test/library-include-tests.sh`'s reader-corners fixture and
      drop the `#59` pointer beside it — that fixture is the **cross-host** case, and it is the one
      that found this.
- [ ] 7.6 Port-reader case in `test/io-ports-tests.sh`: `read` over a file whose text contains both
      comment forms.
- [ ] 7.7 Bar-quoted identifier cases beside the write-style ones: `|foo bar|` reads, `(eq? '|foo|
      'foo)`, the two escapes, `write` emitting bars, `display` not, and the write/read round trip.
      Include a case asserting an ORDINARY symbol still prints bare — that is what says the
      predicate is not over-firing.

## 8. Docs

- [ ] 8.1 `docs/PROJECTS.md`: the Numbers paragraph says what `1/2` now does, and the reader
      limitations lose the comment entries.
- [ ] 8.2 `docs/MODULES.md`: the `include-ci` note explains that a bar-quoted symbol still folds,
      and why that is a separate remaining gap.
- [ ] 8.3 Note in the change what the release notes for the first tag should say: `1/2` used to read
      as a symbol and now reports, and `write` now bar-quotes symbols that need it.
- [ ] 8.4 `docs/r7rs/` cross-references: §7.1.1 identifiers and §7.1.2 comments are now implemented;
      §6.2.5 prefixes are implemented and rationals deliberately are not.

## 9. Regen, suites, close-out

- [ ] 9.1 `make regen` (~12 min), watching for the two-pass prelude/bootstrap ordering case.
- [ ] 9.2 `./run-all-tests.sh`, then `./run-dev-tests.sh` (individually if a suite outlives the
      command timeout).
- [ ] 9.3 Re-record `test/module-scaffold-baseline.sha256` **with the delta explained** per that
      script's protocol: the reader is in `(scheme base)`, so every program's IR moves. Confirm the
      move is confined to the reader — the cross-host suites (`self-emit-equiv`, `dump-parity`,
      `prelude-base-run-tests`) must still pass, which is what says the two readers still agree.
- [ ] 9.4 Measure `emit run` on a large source before and after (design's last risk): the prefix
      scan is on the compiler's own hot path. Record it in `docs/PERFORMANCE.md` only if it shows.
- [ ] 9.5 Commit, then `test/trust-check.sh` (it `[SKIP]`s while `bootstrap/` is dirty).
- [ ] 9.6 `openspec validate --all`, including the main specs.
- [ ] 9.7 Reference both issues from the commit (`Fixes #59`, `Fixes #25`) and comment on #27 that
      its §2 reader-grammar overlap with #25 is closed, leaving it the representation question.
      Note in #25's close that the non-finite half was already done by `numeric-conformance` — its
      body still describes it as open.
- [ ] 9.8 File the follow-up this change deliberately leaves: a **fold-aware read** entry, which is
      what `include-ci` needs to stop folding bar-quoted symbols (design D7). The `include-ci` note in
      `docs/MODULES.md` and the limit comment in `src/core.ss` should point at it.
