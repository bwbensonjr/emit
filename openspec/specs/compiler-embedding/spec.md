...
unterminated construct is reported, not read as end of input"). This is a directional divergence, not
a disagreement about the grammar — both identify the same construct as unterminated at the same
position, and they differ only in what an unterminated construct means to their caller. A host
reading from a stream can supply more input, so the useful answer is "keep reading"; a source file has
no more input to supply, so the useful answer is a diagnostic naming what was left open.
