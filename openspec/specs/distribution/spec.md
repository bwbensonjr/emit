...
installation be self-sufficient for every standard library it ships (binary, default manifest,
and library sources placed where the binary's own manifest lookup will find them), the
`make install` target that produces that layout under a `PREFIX`/`DESTDIR` staging convention,
and the guarantee that installing an Emit leaves the from-source developer workflow untouched.
