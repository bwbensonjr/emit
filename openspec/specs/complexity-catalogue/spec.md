# complexity-catalogue Specification

## Purpose

Defines the reproducible catalogue tool and persisted document that classify the
repository's git-tracked code by component, role, and language so that hand-authored
complexity is never conflated with generated IR, vendored third-party code, imported
reference material, or OpenSpec tracking. Covers the role taxonomy, the single
maintainable classification policy, idempotent regeneration of `docs/COMPLEXITY.md`,
and conformance with the project's tooling-observability output conventions.
