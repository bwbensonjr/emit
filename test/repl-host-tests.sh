...
# NB: a top-level `define` echoes NOTHING (change: unspecified-value), so expectations
# below carry only the non-define forms' values.  Where the old define echo was the only
# evidence for what a test asserts (the heap-survival pair), an explicit bare-name form
# was added to keep that evidence.
...
check cross-module-recursion "720" <<'EOF'
...
check symbol-eq-persist "#t" <<'EOF'
(define s (quote a))
(define t (quote b))
(eq? (quote a) s)
EOF

check trap-isolation "$(printf '25\n!trap: arity error: expected 1 argument(s), got 2\n81')" <<'EOF'
(define (f n) (* n n))
(f 5)
(f 1 2)
(f 9)
