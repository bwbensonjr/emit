...
(define-library (emit internal)
  ...
  (begin
    ...
    (define (rd-hash s n i ci) (if (<= n i) (rd-fail (quote rd-eof) (- i 1)) (let ((k (char->integer (string-ref s i)))) (cond ((= k 116) (cons #t (+ i 1))) ((= k 102) (cons #f (+ i 1))) ((= k 92) (rd-char s n i)) ((= k 40) (let ((r (rd-list s n (+ i 1) (quote ()) ci))) (if (rd-fail? (cdr r)) r (cons (list->vector (car r)) (cdr r))))) ((= k 59) (let ((r (rd-datum s n (rd-skip-ws s n (+ i 1)) ci))) (if (rd-fail? (cdr r)) r (rd-datum s n (rd-skip-ws s n (cdr r)) ci)))) ((and (= k 117) (< (+ i 2) n) (= (char->integer (string-ref s (+ i 1))) 56) (= (char->integer (string-ref s (+ i 2))) 40)) (let ((r (rd-list s n (+ i 3) (quote ()) ci))) (if (rd-fail? (cdr r)) r (cons (list->bytevector (car r)) (cdr r))))) (else (let ((j (rd-token-end s n i))) (let ((v (rd-number (substring s (- i 1) j) 10))) (if (rd-number-reason? v) (rd-fail v (- i 1)) (cons v j)))))))))
    ...
    (define (rd-list s n i acc ci) (let ((j (rd-skip-ws s n i))) (cond ((rd-fail? j) (cons (quote rd-block-comment) j)) ((< j n) (cond ((let ((c (char->integer (string-ref s j)))) (or (= c 41) (= c 93))) (cons (reverse acc) (+ j 1))) ((rd-datum-comment? s n j) (let ((r (rd-datum s n (rd-skip-ws s n (+ j 2)) ci))) (if (rd-fail? (cdr r)) r (rd-list s n (cdr r) acc ci)))) ((rd-dot? s n j) (let ((r (rd-datum s n (rd-skip-ws s n (+ j 1)) ci))) (if (rd-fail? (cdr r)) r (let ((j2 (rd-skip-ws s n (cdr r)))) (if (rd-fail? j2) (cons (quote rd-block-comment) j2) (cons (rd-append-reverse acc (car r)) (+ j2 1))))))) (else (let ((r (rd-datum s n j ci))) (if (rd-fail? (cdr r)) r (rd-list s n (cdr r) (cons (car r) acc) ci)))))) (else (cons (reverse acc) j)))))
    (define (rd-datum s n i ci) (if (and (<= 0 i) (< i n)) (let ((k (char->integer (string-ref s i)))) (cond ((= k 40) (rd-list s n (+ i 1) (quote ()) ci)) ((= k 91) (rd-list s n (+ i 1) (quote ()) ci)) ((= k 39) (rd-quote s n (+ i 1) ci)) ((= k 96) (rd-quasi s n (+ i 1) ci)) ((= k 44) (rd-unquote s n (+ i 1) ci)) ((= k 34) (rd-string s n (+ i 1))) ((= k 35) (rd-hash s n (+ i 1) ci)) ((= k 124) (rd-bar s n (+ i 1) i)) ((or (= k 41) (= k 93)) (rd-fail (quote rd-unexpected) i)) (else (rd-atom s n i ci)))) (if (< i 0) (cons (quote rd-block-comment) i) (rd-fail (quote rd-eof) i))))
