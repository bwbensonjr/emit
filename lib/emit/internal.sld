...
(define-library (emit internal)
  ...
  (begin
    ...
    (define (rd-atom s n i) (let ((j (rd-token-end s n i))) (if (= i j) (rd-fail (quote rd-unexpected) i) (let ((tok (substring s i j))) (let ((v (rd-number tok 10))) (cond ((eq? v (quote rd-not-a-number)) (cons (string->symbol tok) j)) ((rd-number-reason? v) (rd-fail v i)) (else (cons v j))))))))
    ...
    (define (rd-hash s n i) (if (<= n i) (rd-fail (quote rd-eof) (- i 1)) (let ((k (char->integer (string-ref s i)))) (cond ((= k 116) (cons #t (+ i 1))) ((= k 102) (cons #f (+ i 1))) ((= k 92) (rd-char s n i)) ((= k 40) (let ((r (rd-list s n (+ i 1) (quote ())))) (if (rd-fail? (cdr r)) r (cons (list->vector (car r)) (cdr r))))) ((= k 59) (let ((r (rd-datum s n (rd-skip-ws s n (+ i 1))))) (if (rd-fail? (cdr r)) r (rd-datum s n (rd-skip-ws s n (cdr r)))))) ((and (= k 117) (< (+ i 2) n) (= (char->integer (string-ref s (+ i 1))) 56) (= (char->integer (string-ref s (+ i 2))) 40)) (let ((r (rd-list s n (+ i 3) (quote ())))) (if (rd-fail? (cdr r)) r (cons (list->bytevector (car r)) (cdr r))))) (else (let ((j (rd-token-end s n i))) (let ((v (rd-number (substring s (- i 1) j) 10))) (if (rd-number-reason? v) (rd-fail v (- i 1)) (cons v j)))))))))
    ...
    (define (rd-quote s n i) (let ((r (rd-datum s n (rd-skip-ws s n i)))) (if (rd-fail? (cdr r)) r (cons (list (quote quote) (car r)) (cdr r)))))
    (define (rd-quasi s n i) (let ((r (rd-datum s n (rd-skip-ws s n i)))) (if (rd-fail? (cdr r)) r (cons (list (quote quasiquote) (car r)) (cdr r)))))
    (define (rd-unquote s n i) (if (and (< i n) (= (char->integer (string-ref s i)) 64)) (let ((r (rd-datum s n (rd-skip-ws s n (+ i 1))))) (if (rd-fail? (cdr r)) r (cons (list (quote unquote-splicing) (car r)) (cdr r)))) (let ((r (rd-datum s n (rd-skip-ws s n i)))) (if (rd-fail? (cdr r)) r (cons (list (quote unquote) (car r)) (cdr r))))))
    ...
    (define (rd-list s n i acc) (let ((j (rd-skip-ws s n i))) (cond ((rd-fail? j) (cons (quote rd-block-comment) j)) ((< j n) (cond ((let ((c (char->integer (string-ref s j)))) (or (= c 41) (= c 93))) (cons (reverse acc) (+ j 1))) ((rd-datum-comment? s n j) (let ((r (rd-datum s n (rd-skip-ws s n (+ j 2))))) (if (rd-fail? (cdr r)) r (rd-list s n (cdr r) acc)))) ((rd-dot? s n j) (let ((r (rd-datum s n (rd-skip-ws s n (+ j 1))))) (if (rd-fail? (cdr r)) r (let ((j2 (rd-skip-ws s n (cdr r)))) (if (rd-fail? j2) (cons (quote rd-block-comment) j2) (cons (rd-append-reverse acc (car r)) (+ j2 1))))))) (else (let ((r (rd-datum s n j))) (if (rd-fail? (cdr r)) r (rd-list s n (cdr r) (cons (car r) acc))))))) (else (cons (reverse acc) j)))))
    (define (rd-datum s n i) (if (and (<= 0 i) (< i n)) (let ((k (char->integer (string-ref s i)))) (cond ((= k 40) (rd-list s n (+ i 1) (quote ()))) ((= k 91) (rd-list s n (+ i 1) (quote ()))) ((= k 39) (rd-quote s n (+ i 1))) ((= k 96) (rd-quasi s n (+ i 1))) ((= k 44) (rd-unquote s n (+ i 1))) ((= k 34) (rd-string s n (+ i 1))) ((= k 35) (rd-hash s n (+ i 1))) ((= k 124) (rd-bar s n (+ i 1) i)) ((or (= k 41) (= k 93)) (rd-fail (quote rd-unexpected) i)) (else (rd-atom s n i)))) (if (< i 0) (cons (quote rd-block-comment) i) (rd-fail (quote rd-eof) i))))
