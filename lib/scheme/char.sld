...
(define-library (scheme char)
  ...
  (export
    ...
    digit-value char-general-category char-upcase char-downcase char-foldcase char-ci=?
    char-ci<? char-ci>? char-ci<=? char-ci>=? string-upcase string-downcase
    string-foldcase string-ci=? string-ci<? string-ci>? string-ci<=? string-ci>=?)
  ...
  (begin
    ...
    (define (uc-map table cp fallback)
      (let loop ((lo 0) (hi (- (quotient (vector-length table) 2) 1)))
        (if
          (> lo hi)
          fallback
          (let* ((mid (quotient (+ lo hi) 2)) (i (* mid 2)) (key (vector-ref table i)))
            (cond
              ((< cp key) (loop lo (- mid 1)))
    ...
    (define (char-numeric? ch) (uc-in-ranges? %unicode-decimal (char->integer ch)))
    (define (char-whitespace? ch)
      (uc-in-ranges? %unicode-whitespace (char->integer ch)))
    (define (char-upper-case? ch) (uc-in-ranges? %unicode-uppercase (char->integer ch)))
    (define (char-lower-case? ch) (uc-in-ranges? %unicode-lowercase (char->integer ch)))
    ...
    (define (uc-char-chain op a b rest)
      (let ((fa (char->integer (char-foldcase a)))
        ...
        (if (not (op fa fb))
            #f
            (if (null? rest) #t (uc-char-chain op b (car rest) (cdr rest))))))
    ...
    (define (uc-string-chain op a b rest)
      (if (not (op (string-foldcase a) (string-foldcase b)))
          #f
          (if (null? rest) #t (uc-string-chain op b (car rest) (cdr rest)))))
